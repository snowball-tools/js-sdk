...
import { turnkeyAttestPasskey, turnkeyLogin } from './helpers'
...
export class EmbeddedAuth extends SnowballAuth<EmbeddedWallet, EmbeddedAuthState> {
  ...
  async verifyOtp(args: { code: string }) {
    ...
    this.setState({
      name: res.value.puser.passkeys.length ? 'wallet-ready' : 'authenticated-no-passkey',
      user: res.value.puser,
    })
    return res
  }
  ...
  async login() {
    ...
    this.setLoading('emb:login', 'Logging in')
  ...
  async createPasskey({ name }: { name?: string }) {
    ...
    this.setState({ name: 'wallet-ready', user: this.state.user })
