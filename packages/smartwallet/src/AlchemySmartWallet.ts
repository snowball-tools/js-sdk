import type { Chain, SmartWalletProviderInfo } from '@snowballtools/types'
import { alchemyGasPolicyId, viemChain } from '@snowballtools/utils'
...
export class AlchemySmartWallet extends LightSmartContractAccount implements SmartWallet {
  ...
  async sendSponsoredUserOperation(
    ...
    try {
      this.provider = this.provider.withAlchemyGasManager({
        policyId: alchemyGasPolicyId(this.chain, this.smartWalletProviderInfo.apiKeys),
      })
